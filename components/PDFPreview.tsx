...
export const PDFPreview: React.FC<PDFPreviewProps> = ({ data, onEdit, lang, onSubmit, isSubmitting }) => {
  ...
  return (
    <div className="max-w-[210mm] mx-auto py-8 px-4 form-step-enter print:p-0 print:max-w-none">
      {/* Action Bar */}
      <div className="no-print flex flex-col sm:flex-row justify-between items-center mb-8 sticky top-4 bg-white/95 backdrop-blur shadow-xl ring-1 ring-gray-900/5 p-4 rounded-xl z-50 gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-indigo-100 p-2 rounded-lg">
            <FileText className="w-5 h-5 text-indigo-600" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-gray-900">{t.actions.exportPdf}</h2>
            <p className="text-xs text-gray-500">Review your information before printing or submitting</p>
          </div>
        </div>
        <div className="flex w-full sm:w-auto gap-3">
          <Button variant="outline" onClick={onEdit} className="flex-1 sm:flex-none border-gray-300 hover:bg-gray-50">
            <Edit className="w-4 h-4 mr-2" /> {t.actions.edit}
          </Button>
          <Button onClick={handlePrint} className="flex-1 sm:flex-none bg-gray-800 hover:bg-gray-700 text-white shadow-lg">
            <Printer className="w-4 h-4 mr-2" /> {t.actions.print}
          </Button>
          {onSubmit && (
            <Button onClick={onSubmit} isLoading={isSubmitting} className="flex-1 sm:flex-none bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-200">
