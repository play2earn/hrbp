...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!isStaff && !authResult.valid) {
    logApiKeyUsageAsync({
      req,
      endpoint: '/api?route=hrms-export',
      method: 'GET',
      statusCode: 401,
  ...
  try {
    ...
    logApiKeyUsageAsync({
      ...
      responseTimeMs: Date.now() - startTime,
      queryParams: req.query as any,
      summary: application_id ? `Exported single candidate (${application_id})` : `Exported batch queue (${payload.length} records)`,
    });
    ...
    logApiKeyUsageAsync({
      ...
      responseTimeMs: Date.now() - startTime,
      queryParams: req.query as any,
      errorMessage: err.message || 'Internal server error during HRMS export',
    });
