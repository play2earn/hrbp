...
// Add `const isForeigner = viewingApp?.form_data?.isThaiNational === false; const lang = isForeigner ? 'en' : 'th';` right after `const fd = viewingApp.form_data || {};` (around line 158)
...
// Special cases
content = content.replace(
  /label=\{fd\.isThaiNational \? 'เลขบัตรประชาชน' : 'หมายเลขหนังสือเดินทาง'\}/,
  "label={lang === 'en' ? 'ID Card / Passport' : (fd.isThaiNational ? 'เลขบัตรประชาชน' : 'หมายเลขหนังสือเดินทาง')}"
);
content = content.replace(
  /label="Work Permit \/ สิทธิ์ทำงานในไทย"/,
  "label={lang === 'en' ? 'Work Permit' : 'Work Permit / สิทธิ์ทำงานในไทย'}"
);
content = content.replace(
  /value=\{fd\.availableToWorkInThailand \? '✅ มีสิทธิ์ทำงานในประเทศไทย' : '⚠️ ยังไม่มีสิทธิ์ทำงานในประเทศไทย'\}/,
...
content = content.replace(
  /value=\{fd\.isThaiNational \? 'ไทย' : 'ต่างชาติ'\}/,
