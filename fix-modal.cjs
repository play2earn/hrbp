...
content = content.replace(/<span className="text-gray-600">ชื่อ: <span/g, "<span className=\"text-gray-600\">{lang === 'en' ? 'Name:' : 'ชื่อ:'} <span");
content = content.replace(/<span className="text-gray-600"><span>อายุ: <span/g, "<span className=\"text-gray-600\"><span>{lang === 'en' ? 'Age:' : 'อายุ:'} <span");
content = content.replace(/<\/span><span>อาชีพ: <span/g, "</span><span>{lang === 'en' ? 'Occupation:' : 'อาชีพ:'} <span");
...
const eduObjMatch2 = `const levelNames: Record<string, string> = {
                        primarySchool: 'ประถมศึกษา (ป.1-6)', juniorHighSchool: 'มัธยมต้น (ม.1-3)',
                        highSchool: 'มัธยมปลาย / ปวช.', vocational: 'ปวส.',
                        bachelor: 'ปริญญาตรี', master: 'ปริญญาโท', phd: 'ปริญญาเอก',
                      };`;
const eduObjReplace2 = `const levelNames: Record<string, string> = lang === 'en' ? {
                        primarySchool: 'Primary School', juniorHighSchool: 'Junior High School',
                        highSchool: 'High School / Voc.Cert.', vocational: 'Higher Vocational',
                        bachelor: 'Bachelor', master: 'Master', phd: 'Ph.D.',
                      } : {
...
content = content.split(eduObjMatch2).join(eduObjReplace2);

// Military mapping
content = content.replace(/value=\{getMilitaryStatusLabel\(fd\.militaryStatus\)\}/g, "value={lang === 'en' ? (fd.militaryStatus === 'ExemptLaw' || fd.militaryStatus === 'ExemptFemale' ? 'Exempted' : fd.militaryStatus) : getMilitaryStatusLabel(fd.militaryStatus)}");

fs.writeFileSync(file, content);
console.log('Done fix-modal.cjs');
